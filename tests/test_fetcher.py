...
def _mock_resp(status_code: int = 200, headers: dict | None = None) -> MagicMock:
    """Create a MagicMock with spec=httpx.Response to catch typos in mock assertions."""
    m = MagicMock(spec=httpx.Response)
    m.status_code = status_code
    m.headers = headers or {"X-MBX-USED-WEIGHT-1M": "1"}
    return m
...
def test_fetch_klines_futures_returns_dataframe():
    mock_resp = _mock_resp(headers={"X-MBX-USED-WEIGHT-1M": "5"})
...
# ---------------------------------------------------------------------------
# Empty-response early-exit paths
# ---------------------------------------------------------------------------

def test_fetch_funding_rate_empty_response_returns_empty_dataframe(monkeypatch):
    """fetch_funding_rate must return a typed empty DataFrame when API returns []."""
    monkeypatch.setattr("binance_bars.fetcher._http_get", lambda url, params: [])
    df = fetch_funding_rate(symbol="BTCUSDT")
    assert isinstance(df, pd.DataFrame)
    assert len(df) == 0
    assert list(df.columns) == ["funding_time", "symbol", "funding_rate", "mark_price"]


def test_fetch_open_interest_empty_response_returns_empty_dataframe(monkeypatch):
    """fetch_open_interest must return a typed empty DataFrame when API returns []."""
    monkeypatch.setattr("binance_bars.fetcher._http_get", lambda url, params: [])
    df = fetch_open_interest(symbol="BTCUSDT", period="5m")
    assert isinstance(df, pd.DataFrame)
    assert len(df) == 0
    assert list(df.columns) == ["timestamp", "symbol", "open_interest", "open_interest_value"]


def test_fetch_basis_empty_response_returns_empty_dataframe(monkeypatch):
    """fetch_basis must return a typed empty DataFrame when API returns []."""
    monkeypatch.setattr("binance_bars.fetcher._http_get", lambda url, params: [])
    df = fetch_basis(symbol="BTC", interval="1d")
    assert isinstance(df, pd.DataFrame)
    assert len(df) == 0
    assert list(df.columns) == ["timestamp", "pair", "futures_price", "index_price",
                                  "basis", "basis_rate"]
